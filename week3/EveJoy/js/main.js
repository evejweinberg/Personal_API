...
function updateAllData(data) {
    for (i in data) {
        // theAnswer00[i] = data[i]['Q1: Where did you get your data from?'];
...
function Vis(num, x, y, started, question) {
    ...
    this.showStatGraphics = function() {
        if (num == 0) {
            ...
            for (var i = 0; i < 16; i++) {
                ...
                if (i < 8) {
                    ...
                    if (theAnswer00[i] != '') {
                        ...
                        text(theAnswer00[i], this.x + 10, this.y + (dimsY / 8 * i) + (dimsY / 16))
                    }
                } else {
                    ...
                    if (theAnswer00[i] != '') {
                        text(theAnswer00[i], this.x + dimsX / 2 + 10, this.y + (dimsY / 8 * [i % 8]) + +(dimsY / 16))
                    }
                }

            }
        } else if (num == 1) {
            q2Results[0] = theAnswer01.filter(function(x) {
                return x == 'yes'
            }).length
            q2Results[1] = theAnswer01.filter(function(x) {
                return x == 'no'
            }).length
            ...
            q2D3 = theAnswer02.filter(function(x) {
                return x == 'd3'
            }).length
            q2CJS = theAnswer02.filter(function(x) {
                return x == 'chartjs'
            }).length
            var totalAnswers = q2CJS+q2D3
            // console.log(totalAnswers)
            var D3height = map(q2D3, 0, theAnswer02.length, 20, dimsY * .9)
            var D3rad = map(q2D3, 0, theAnswer02.length, 0, 120)
            var CJSheight = map(q2CJS, 0, theAnswer02.length, 20, dimsY * .9)
            var CJSrad = map(q2CJS, 0, theAnswer02.length, 0, 120)
            fill(colors[1])
            stroke(colors[1])
            strokeWeight(12.0);
            strokeCap(ROUND);
            ...
            q4Results[0] = theAnswer03.filter(function(x) {
                return x != "el.style.display = 'none'"
            }).length
            q4Results[1] = theAnswer03.filter(function(x) {
                    return x == "el.style.display = 'none'"
                }).length
        ...
        if (num == 4) {
          ...
          noStroke()
            ...
            for (i in theAnswer04) {
                q5Radius[i] = map(theAnswer04[i], 1, 10, dimsX * .31, 0)
                fill(colors[3])
